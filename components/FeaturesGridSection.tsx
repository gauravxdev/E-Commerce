...
export default function FeaturesGridSection({ config }: { config?: SectionConfig }) {
  const bgColor = config?.bgColor || "";
  const textColor = config?.textColor || "text-gray-900";

  return (
    <section className={`w-full max-w-7xl mx-auto px-6 py-12 ${bgColor}`}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2 rounded-[2rem] overflow-hidden bg-gradient-to-br from-[#e0f2fe] via-[#f0f9ff] to-[#e0f7fa] relative flex flex-col md:flex-row items-center p-8 md:p-12 h-auto md:h-[400px]">
          <div className="md:w-1/2 z-10">
            ...
            <h3 className={`text-3xl md:text-5xl font-bold ${textColor} mb-6 leading-tight`}>
              {config?.title || "Simplicity of\ntransforming"}
            </h3>
            <p className="text-gray-600 mb-8 max-w-sm">
              {config?.subtitle || "Discover the perfect balance of deep bass, clear minds and crisp highs"}
            </p>
            <button className="flex items-center space-x-3 text-sm font-semibold text-gray-900 group">
              <span className="bg-white px-6 py-3 rounded-full hover:bg-gray-50 transition-colors shadow-sm">{config?.buttonText || "Read More"}</span>
        ...
        <div className="rounded-[2rem] overflow-hidden bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#eff6ff] relative p-8 md:p-12 h-[400px] flex flex-col">
          ...
          <h3 className={`text-3xl font-bold ${textColor} mb-4 leading-tight z-10`}>
            Sound<br/>perfection
          </h3>
